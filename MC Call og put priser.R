...
MC_Putprice_method2<-function(s0,r,sigma,n_sim,T,n_paths,K,dt){
...
for (i in 1:n_paths){
  simulated_paths<-Method2(x0=s0,mu=r,sigma=sigma,n_sim=n_sim,time_vector = time_vector)
...
return(mean(montecarlo_paths[nrow(montecarlo_paths),]))
}

#Montecarlo Call pris med metode 1 uden varians reduktion.

MC_Callprice_method2<-function(s0,r,sigma,n_sim,n_paths,K,T,dt){
